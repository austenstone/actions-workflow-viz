...
async function resolveToken() {
  ...
  if (env) return env;
  try {
    const { stdout } = await pExecFile("gh", ["auth", "token"], { timeout: 5e3 });
...
function getOctokit() {
  if (!clientPromise) {
    clientPromise = (async () => {
      const auth2 = await resolveToken();
...
function nodeStatusFromLegs(legs) {
  ...
  if (legs.some(
    (l) => l.status === "queued" || l.status === "waiting" || l.status === "requested" || l.status === "pending"
  )) {
...
function cacheYaml(key, value) {
  if (yamlCache.size >= YAML_CACHE_MAX) {
    const oldest = yamlCache.keys().next().value;
    if (oldest !== void 0) yamlCache.delete(oldest);
  }
  yamlCache.set(key, value);
}
async function fetchRunGraph({ repo, runId }) {
...
var loadInputSchema = {
  ...
  properties: {
    ...
    runUrl: {
      type: "string",
      description: "Full run URL, e.g. https://github.com/owner/repo/actions/runs/123. Alternative to repo+runId."
    }
  }
};
var sessionLog;
